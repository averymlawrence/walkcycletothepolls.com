...
- **Favicon (Sept 2026, updated)** — Site root has `favicon.ico` (16/32/48px), `favicon-16x16.png`, `favicon-32x32.png`, `apple-touch-icon.png` (180px). Every page's `<head>` links these with root-relative paths (`/favicon.ico`, etc.) so they resolve correctly from any folder depth. Currently generated from `graphics/WCTTP-favicon.svg` — a dedicated icon-only mark (the wave pattern from the logo, no wordmark) Avery supplied specifically for this purpose, rendered white-on-black and flattened to a square canvas via ImageMagick. This replaced an earlier first pass that used the full `WCTTP-logo-Black.png` wordmark, which was illegible at 16-32px; the dedicated icon mark reads clearly even at 32px. If Avery supplies another favicon source in the future, regenerate all four files from it the same way (flatten to square, black background, sizes 16/32/48/180) rather than editing the PNGs by hand. Leftover intermediate files (`graphics/favicon-source-padded.png`, `favicon-48x48.png`, and duplicate copies in `graphics/`) are harmless and untracked — the canonical files are the ones at the site root.

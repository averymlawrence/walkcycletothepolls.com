...
- **Funders & Partners (about.html, updated Sept 2026)** — Current order: Stamps School of Art & Design (first), Arts Initiative, DCE, Ann Arbor District Library, Michigan Animation Club, NowWhat?!. `images/grantors/Stamps-School-logo-white.svg` is now sourced from Avery's `STAMPS_Informal.CMYK.white.svg` (has a small yellow accent on the wave mark, not pure white — replaced the earlier plain-white version). Michigan Animation Club's logo (`Michigan-Animation-Club_LOGO.png`) is shown directly on the black section background at 84px tall (50% bigger than the original 56px) — it is NOT wrapped in a white box; the PNG already has a transparent square around its circular badge, so it sits cleanly on black. `images/grantors/NowWhat-logo.svg` (from Avery's `NowWhat_Vertical_Color.svg`) was added last, as a plain (non-anchor) image — no confirmed URL for NowWhat yet, so it isn't a link. If Avery gives you their website, wrap it in an `<a>` like the others.
...
- **Favicon (Sept 2026, updated twice)** — Site root has `favicon.ico` (16/32/48px), `favicon-16x16.png`, `favicon-32x32.png`, `apple-touch-icon.png` (180px). Every page's `<head>` links these with root-relative paths (`/favicon.ico`, etc.) so they resolve correctly from any folder depth. Canonical source is now `graphics/WCTTP-favicon.png` (also kept alongside `graphics/WCTTP-favicon.svg`) — a flattened black-on-transparent icon mark (the wave pattern from the logo, no wordmark) Avery supplied specifically for this purpose. The small PNGs/ICO are generated by squaring it on a **transparent** background (not flattened onto black) so the wave strokes stay visible at every edge; only `apple-touch-icon.png` gets an opaque backdrop (`#83eaf7`) since Apple doesn't handle transparency well. An earlier pass rendered the icon flattened onto an opaque black square, which clipped/hid the top and bottom wave rows when squared off — don't repeat that; always square favicon source art on a transparent canvas, and only flatten to an opaque color for the apple-touch-icon. If Avery supplies another favicon source in the future, prefer using their flattened PNG directly over re-deriving from an SVG. Leftover intermediate files (`graphics/favicon-source-padded.png`, `favicon-48x48.png`, and duplicate copies in `graphics/`) are harmless and untracked — the canonical files are the ones at the site root.
...
- **Default brush size (Sept 2026)** — `brushSize` in `tool/index.html` now defaults to 10px (was 1px) for easier drawing; changed in three places: the `#brushSize`/`#brushSizeMobile` input `value` attributes and the `let brushSize` module variable. Still clamped 1-20 via the existing input handlers.
- **4-frame linked-frame sync fixed (Sept 2026)** — The actual pixel-syncing logic in `syncLinkedFrames()` (`tool/index.html`) originally auto-copied frame 1 ↔ frame 3 (0-indexed 0 ↔ 2) whenever either was edited, and the timeline badge showed "=1" on frame 3's thumbnail — both leftover from the same "1 & 3 are the same" mistake caught earlier. Fixed to sync frame 2 ↔ frame 4 (0-indexed 1 ↔ 3) instead, matching the corrected walk-cycle structure (two different contact poses at frames 1 & 3, one repeated passing pose at frames 2 & 4). The badge now shows "=2" on frame 4's thumbnail instead of "=1" on frame 3's. If this ever needs revisiting, the badge lives in `buildTimeline()` and the actual copy logic in `syncLinkedFrames()` — keep them in sync with each other.

- **Home page sun character (Sept 2026)** — `.hero__sun` in `css/style.css`, a `<picture>` at the top of the `.hero` in `index.html`. Width is `min(270px, 60vw)` = 75% of `.hero__logo`'s `min(360px, 80vw)` — keep that ratio if the logo size changes. `z-index: 1` so the logo (`z-index: 2`) sits in front. On phones (≤600px) the sun shrinks to 28vw and is anchored to the logo's top-right corner via `.hero__logo-wrap` (so the logo overlaps only its lower-left rays on every screen height); on desktop the wrapper is static and the sun stays pinned to the hero's corner. Serves `images/sun-character/SUN-v2-BLINKING.webp` (680 KB animated WebP made from Avery's 3.7 MB GIF with `gif2webp -lossy`), GIF as fallback, and `SUN-v2-static.webp` for prefers-reduced-motion. The 13 MB spritesheet PNG in that folder isn't used by the site.
- **How to Submit page + Sprite Sheet Maker (Sept 2026)** — `walking/how-to-submit.html` (3 submission options, spec summary, template downloads from `software-templates/`, canvas-setup animation, race backgrounds) and `walking/sprite-sheet-maker.html` + `js/spritesheet-maker.js` (in-browser, no uploads: one 8-frame GIF or up to 8 PNGs, each 800×800 with transparency → 6400×800 PNG; includes its own small GIF decoder, verified pixel-identical to ImageMagick's coalesce). Both are in the Walking dropdown on every page. The RoughAnimator template is a folder, so it's served as `software-templates/WCTTP-26-RoughAnimator-TEMPLATE.ra.zip` — re-zip it if Avery updates the template. The canvas-setup animation is rebuilt from Avery's split elements in `images/submission-instructions/canvas-setup-GIF-elements/` as two stacked layers (`.canvas-anim` in `css/style.css`): `canvas-setup-static.webp` (labels + checkerboard square composited at 2x, 1600×1736, transparent bg, ~90 KB) and `canvas-setup-characters.webp` (only the characters, animated, ~1.7 MB, positioned 16.5% left / 15.207% top / 83.5% wide = the square's spot in the original 800×868 layout). Total ~1.8 MB vs the original 11.7 MB GIF, which is no longer used by the site; race background previews are resized `-preview.jpg` copies, downloads link the originals.

## Things still to do / pending decisions
- **`verification.html`** — Root-level page (added by Jacob Robinson, commit `0b0e641`) with a username/password form that submits to `https://dfiv0rwmt3.execute-api.us-east-1.amazonaws.com/Default/verification/site` (same API host as the `/Default/characters` endpoint used by `walking/upload-walk-cycle.html`). Confirmed intentional as of Sept 2026 — Avery says Jacob is using it as part of building the Walk Cycle to the Polls game backend. It's a WIP/unstyled page and not linked from the site nav, so it's not something a visitor would stumble into. Do not remove or "fix" this without checking with Avery/Jacob first.
